...
export function ConditionsSection({ tree, onChange }: ConditionsSectionProps) {
  ...
  const actualTab = activeTab === "summary" ? "builder" : activeTab;
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center justify-between px-6 pt-6 pb-2 shrink-0">
        <div>
          ...
          <p className="text-[13px] text-(--fg-muted) m-0">
            Build the predicate that fires this rule. {stats.total} conditions
        ...
        <div className="flex gap-1 bg-(--bg-inset) p-0.5 rounded-lg">
          {(
            ...
            <button
              ...
              style={{
                background:
                  actualTab === v.id ? "var(--bg-elev)" : "transparent",
                color: actualTab === v.id ? "var(--fg)" : "var(--fg-muted)",
                boxShadow:
                  actualTab === v.id ? "0 1px 2px rgba(0,0,0,0.06)" : "none",
              }}
            >
              {v.label}
            </button>
      ...
      {actualTab === "builder" && (
        <div className="p-6 overflow-auto flex-1 min-h-0 border-t border-(--border) flex flex-col gap-4">
          <GuardConditionCard />
...
export function moveNode(
  ...
  const newTree = structuredClone(tree) as Condition;

  const { parent: fromParent, index: fromIndex } = getParentAndIndex(
    newTree,
    fromPath,
  );
  ...
  if (position === "inside") {
    ...
    const { parent: toParent, index: toIndex } = getParentAndIndex(
      newTree,
      adjustedToPath,
    );
...
export function ConditionsNested({
  tree,
  onChange,
}: {
  tree: Condition;
...
type InsertFieldHandler = (
  field: { path: string; description: string; type: string },
  toPath: number[],
  pos: "before" | "after" | "inside",
) => void;

// Wires up the three drag-and-drop roles a condition group plays: a drag
// source (so it can be reordered as a unit), a drop target on its own card
// (for before/after placement relative to siblings), and a drop target on
// its children container (for dropping "inside" as a new child).
function useCondGroupDnd({
  path,
  depth,
  onMoveNode,
  onInsertField,
  ...
  onInsertField: InsertFieldHandler;
}) {
  ...
  const [{ isOverGroup, canDropGroup }, dropRefGroup] = useDrop({
    ...
    drop: (item: unknown, monitor) => {
      ...
      if (f) {
        onInsertField(
          f as { path: string; description: string; type: string },
          path,
          pos,
        );
        return;
  ...
  return {
    ...
    containerRef,
    dropRefChildren,
    showHighlight: isOverGroup && canDropGroup,
    isOverChildren,
    canDropChildren,
  };
}

function CondGroupHeader({
  op,
  childCount,
  depth,
  dragRef,
...
function CondGroupChildren({
  ...
  fields: FieldDef[];
  borderColor: string;
  dropRefChildren: (node: HTMLDivElement | null) => void;
  isOverChildren: boolean;
  canDropChildren: boolean;
  onMoveNode: MoveNodeHandler;
  onInsertField: InsertFieldHandler;
  updateChild: (idx: number, next: Condition | null) => void;
}) {
  ...
  return (
    <div
      ...
      style={{
        ...
        marginLeft: 12,
        position: "relative",
        minHeight: childCount === 0 ? "auto" : "20px",
        padding: isOverChildren && canDropChildren ? "6px" : "0px",
        background:
          isOverChildren && canDropChildren
...
function CondGroup({
  ...
  const {
    ...
    isOverChildren,
    canDropChildren,
  } = useCondGroupDnd({ path, depth, onMoveNode, onInsertField });
  ...
  return (
    ...
    >
      ...
      <CondGroupChildren
        ...
        dropRefChildren={dropRefChildren}
        isOverChildren={isOverChildren}
        canDropChildren={canDropChildren}
        onMoveNode={onMoveNode}
        onInsertField={onInsertField}
...
function Cond({
  ...
  const ops = fieldMeta
    ...
    : ["=", "\u2260", ">", "<"];
  const isEnum =
    fieldMeta?.type === "enum" || fieldMeta?.type === "boolean_like";
  ...
  return (
    <div
      ...
      style={{
        display: "grid",
        gridTemplateColumns:
          "auto minmax(180px, 1.4fr) minmax(80px, 0.6fr) minmax(140px, 1.4fr) auto",
        gap: 6,
        alignItems: "center",
        padding: 6,
...
export function GuardConditionCard() {
  ...
  return (
    <div className="rounded-lg border border-(--border) p-4 bg-(--bg-elev)">
...
export function ConditionsSummary({ tree }: { tree: Condition }) {
      ...
      if (node.children) {
        ...
        const innerWalk = (n: Condition) => {
          if (n.type === "cond" && n.field) {
            ...
            childSummaries.push(
              `${meta?.description || n.field} ${n.op} ${n.value}`,
            );
          }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      <div>
        ...
        <p className="text-[13px] text-(--fg-muted) m-0">
          An overview of the current rule configuration and draft state.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-(--bg-elev) border border-(--border) p-4 rounded-xl shadow-sm">
          <h3 className="text-[13px] font-semibold mb-3 flex items-center gap-2">
            <Icon name="info" size={14} className="text-primary" />
            Rule Metadata
          </h3>
          <div className="space-y-2 text-[12px]">
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Name</span>
              <span className="font-medium">
                {draft.identity.name || "Unnamed Rule"}
              </span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Code</span>
              <span className="font-mono font-medium">
                {draft.identity.code || "N/A"}
              </span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Version</span>
              <span className="font-mono">
                {draft.identity.version || "1.0.0"}
              </span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Author</span>
              <span>{draft.identity.author || "N/A"}</span>
            </div>
            {draft.identity.description && (
              <div className="pt-1">
                ...
                <p className="text-(--fg-muted) m-0 bg-(--bg-inset) p-2 rounded text-[11px] leading-relaxed">
                  {draft.identity.description}
                </p>
              </div>
            )}
        ...
        <div className="bg-(--bg-elev) border border-(--border) p-4 rounded-xl shadow-sm">
          ...
          <div className="space-y-2 text-[12px]">
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Execution Mode</span>
              <span className="font-semibold capitalize text-(--fg)">
                {draft.policy.mode}
              </span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Rollout Rate</span>
              <span>{draft.policy.rollout}%</span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Target Channels</span>
              <div className="flex gap-1 flex-wrap">
                {draft.channels.length > 0 ? (
                  draft.channels.map((ch) => (
                    <span
                      key={ch}
                      className="bg-(--bg-inset) px-1.5 py-0.5 rounded text-[10px] uppercase font-semibold text-(--fg-subtle)"
                    >
                      {ch}
                    </span>
                  ))
            ...
            <div className="pt-1">
              <span className="text-(--fg-muted) block mb-1">Schedule</span>
              <div className="text-[11px] font-mono bg-(--bg-inset) p-2 rounded flex flex-col gap-1 text-(--fg-muted)">
                <div>From: {draft.policy.schedule_from || "Immediate"}</div>
                <div>To: {draft.policy.schedule_to || "Indefinite"}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-(--bg-elev) border border-(--border) p-4 rounded-xl shadow-sm">
        <h3 className="text-[13px] font-semibold mb-3 flex items-center gap-2">
          <Icon name="rule" size={14} className="text-primary" />
        ...
        <div className="text-[12px] text-(--fg-muted) space-y-1 bg-(--bg-inset) p-3 rounded-lg border border-(--border-strong)">
          {summaryEntries.length > 0 ? (
            ...
            <div className="italic py-1 text-center">No conditions defined</div>
          )}
        </div>
      </div>
        ...
        <div className="bg-(--bg-elev) border border-(--border) p-4 rounded-xl shadow-sm">
          ...
          <div className="space-y-2 text-[12px]">
            <div className="flex justify-between items-center pb-1">
              ...
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-semibold ${draft.evaluationCondition === true ? "bg-(--action-allow-bg) text-(--action-allow)" : "bg-(--action-review-bg) text-(--action-review)"}`}
              >
                {draft.evaluationCondition === true
                  ? "Always True"
                  : "Custom Filter"}
              </span>
            </div>
            {draft.evaluationCondition !== true && (
              <div className="bg-(--bg-inset) p-2 rounded font-mono text-[11px] max-h-24 overflow-auto border border-(--border-strong)">
                {JSON.stringify(draft.evaluationCondition, null, 2)}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-(--bg-elev) border border-(--border) p-4 rounded-xl shadow-sm">
          ...
          <div className="space-y-2 text-[12px]">
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              ...
              <span
                className={`font-semibold capitalize text-[13px] ${
                  draft.enforcement.action === "block"
                    ? "text-(--action-block)"
                    : draft.enforcement.action === "review"
                      ? "text-(--action-review)"
                      : "text-(--action-allow)"
                }`}
              >
                {draft.enforcement.action.toUpperCase()}
              </span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Score Impact</span>
              <span className="font-semibold">
                {draft.enforcement.score_impact}
              </span>
            </div>
            <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
              <span className="text-(--fg-muted)">Severity</span>
              <span className="capitalize">{draft.enforcement.severity}</span>
            </div>
            {draft.enforcement.cooldown_seconds > 0 && (
              <div className="flex justify-between border-b border-(--border-faint) pb-1.5">
                <span className="text-(--fg-muted)">Cooldown</span>
                <span>{draft.enforcement.cooldown_seconds}s</span>
              </div>
            )}
          </div>
        </div>
...
export function ConditionsTreeView({ tree }: { tree: Condition }) {
  const render = (node: Condition, depth: number): ReactElement => {
    ...
    return (
      ...
      >
        ...
        {node.children?.map((c) => (
          <div key={c.id}>{render(c, depth + 1)}</div>
        ))}
      </div>
    );
  };
...
export function ConditionsCodeView({ tree }: { tree: Condition }) {
  const evaluationCondition = useRuleStore((s) => s.draft.evaluationCondition);
