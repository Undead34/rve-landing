...
export function EventBuilder({ onEvaluate, evaluating, recentSimulations }: EventBuilderProps) {
    ...
    <div className="flex flex-col overflow-hidden">
      <div className="flex items-center justify-between gap-4 px-6 py-[14px] border-b border-[var(--border)] bg-[var(--bg-elev)]">
        ...
        <div className="flex gap-2">
          ...
          <Button kind="accent" icon="play" onClick={onEvaluate} disabled={evaluating}>
        ...
        <div className="border-r border-[var(--border)] p-3 bg-[var(--bg-elev)] overflow-y-auto">
          ...
          <div className="flex flex-col gap-[2px]">
            {sections.map((s) => (
              ...
              >
                <span>{s.label}</span>
                <span className="ml-auto text-[10px] text-[var(--fg-subtle)] font-mono">{s.count}</span>
              </div>
            ))}
          </div>
            ...
            {recentSimulations.map((s) => (
              ...
              >
                <div className="flex justify-between items-center gap-1">
                  ...
                  <Badge kind={s.outcome as "allow" | "review" | "block" | "tag_only"} dot>{s.outcome}</Badge>
                </div>
                <div className="text-[11px] mt-1 text-[var(--fg-muted)] leading-relaxed truncate">{s.label}</div>
                <div className="text-[10px] text-[var(--fg-subtle)] mt-[2px]">{s.when} · score {s.score}</div>
              </div>
            ))}
          </div>
        </div>
...
function EventSectionForm({ section }: { section: string }) {
  const sections: Record<string, { title: string; subtitle: string; fields: FormField[] }> = {
    header: {
      title: "Header",
      subtitle: "Identifies the event and the channel it originates from.",
      fields: [
        { label: "event_id", type: "text", value: "evt_8af3b2c4d1", mono: true, hint: "Auto-generated if empty" },
        { label: "event.type", type: "select", value: "transaction", options: ["transaction", "login", "signup", "password_reset", "withdrawal"] },
        { label: "event.channel", type: "select", value: "web", options: ["web", "mobile", "api", "branch", "atm", "callcenter"] },
        { label: "event.timestamp", type: "datetime", value: "2026-05-19T10:42:18", hint: "Defaults to now if omitted" },
      ],
    },
    context: {
      title: "Context",
      ...
      fields: [
        { label: "customer.id", type: "text", value: "cus_94k2lf3a", mono: true },
        { label: "customer.age_days", type: "number", value: 412 },
        { label: "customer.kyc_level", type: "select", value: "L2", options: ["L0", "L1", "L2", "L3"] },
        { label: "customer.risk_segment", type: "select", value: "medium", options: ["low", "medium", "high"] },
        { label: "session.id", type: "text", value: "sess_b9d31fa2", mono: true },
        { label: "session.ip", type: "text", value: "85.214.132.117", mono: true },
      ],
    },
    features: {
      title: "Features",
      ...
      fields: [
        ...
        { label: "features.merchant_category_risk", type: "number", value: 0.36 },
    ...
    signals: {
      ...
      fields: [
        { label: "signals.velocity_1h", type: "number", value: 6, hint: "⚠ Above typical threshold (5)" },
        { label: "signals.velocity_24h", type: "number", value: 14 },
        { label: "signals.device_fingerprint", type: "text", value: "fp_a8e2d4f9c1b3", mono: true },
        { label: "signals.ip_country", type: "text", value: "NL", mono: true },
        { label: "signals.ip_risk_score", type: "number", value: 0.72, hint: "Hosting provider detected" },
      ],
    },
    payload: {
      title: "Payload",
      ...
      fields: [
        ...
        { label: "payment.method", type: "select", value: "card", options: ["card", "ach", "wire", "crypto", "pix"] },
        { label: "payment.card_bin", type: "text", value: "411111", mono: true },
        { label: "payment.card_country", type: "text", value: "US", mono: true },
        { label: "payment.merchant_id", type: "text", value: "mrch_8a3df21", mono: true },
        { label: "payment.merchant_name", type: "text", value: "OnlineRetailer Inc." },
        { label: "payment.merchant_mcc", type: "text", value: "5732", mono: true, hint: "Electronics" },
      ],
    },
  }
  ...
  return (
    <div className="flex flex-col gap-4" style={{ maxWidth: 720 }}>
      ...
      <div className="grid grid-cols-2 gap-3">
        {s.fields.map((f: FormField) => (
          <div key={f.label} className="flex flex-col gap-1">
            ...
            {f.hint && (
              <div className="text-[10px] text-[var(--fg-subtle)]">{f.hint}</div>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-[6px] mt-2">
        <Button size="sm" icon="plus">Add custom field</Button>
        <Button size="sm" kind="ghost">View as JSON</Button>
      </div>
    </div>
  )
}
